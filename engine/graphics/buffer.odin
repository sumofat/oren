...
buf_push :: proc(buffer : ^Buffer($element_type),element : element_type) -> u64
{
    ...
    buffer.current_id = index + 1;//cast(u64)len(buffer.buffer);
    return index;
}

buf_get :: proc(buffer : ^Buffer($element_type),index : u64,$type : typeid) -> (type)
{
    assert(buffer != nil);
...
buf_chk_out :: proc(buffer : ^Buffer($element_type),index : u64,$type : typeid) -> (^type)
{
    assert(buffer != nil);
